...
$universityrows  = $wpdb->get_results("SELECT t1.nuniversity_id, t1.vuniversity_title, t2.npdf_id, t2.vpdf_data, t2.vimage_data, t2.vpdf_type
					FROM $table_name1 t1
...
$courserows  = $wpdb->get_results("SELECT t1.ncourse_id, t1.vcourse_title, t2.npdf_id, t2.vpdf_data, t2.vimage_data, t2.vpdf_type
...
if($tab_type == 'university-id'){
    foreach($universityrows as $ukey => $uvalue){ 
	    if($uvalue['nuniversity_id'] == $tab_type_id){
	       // $heading = '<a href="'.$dbsiteurl.'/format-guidelines">Format & Guidelines</a> - '.$uvalue['vuniversity_title']; break;
	        $heading = '<a href="'.$dbsiteurl.'/format-guidelines">University Formats & Guidelines</a> - '.$pg_param; break;
	    } 
    }    
}
else{
    foreach($courserows as $ckey => $cvalue){ 
        if($cvalue['ncourse_id'] == $tab_type_id){
            // $heading = '<a href="'.$dbsiteurl.'/samples">Samples</a> - '.$cvalue['vcourse_title']; break;
            $heading = '<a href="'.$dbsiteurl.'/samples">Research Writing Sample</a> - '.$pg_param; break;
        }
    }
}
...
//      tab_type = e.path[0].attributes[2].name;
    ...
    if(tab_type == 'university-id'){
		var notihtml = '<div class="col-12 text-start"><div class="alert alert-warning text-center" role="alert"><strong><a href="http://phdguides.org/samples/" target="_blank" class="btn btn-primary" style="font-weight:bold;">Also see our Research Writing Sample</a></strong></div></div>';
...
//      university_id = e.path[0].attributes[2].value;
                    		...
                    		for (const optid in pdfv) {
                        		if(optid == optv.noption_id){
									...
									if('<?php echo $optid_param; ?>' == optv.noption_id){ 
                                        options_section += '<input type="radio" for="regForm" value="'+optv.noption_id+'" name="seloptions" seluniversity="'+university_id+'" checked="true">';
                        		    }
                        		    else{
        ...
        options_section     +=  '</div>';
	  	...
	  	// options_section 	+=	'<sml id="nextbtn-text" style="font-weight:bold; font-size: 12px;"></sml>';
			...
			if(optv.voption_type == 'sample'){
            	if(tab_type == 'sample-id'){
            		let pdfdataa = course_data_array.map((univ,unii) => {
            		    console.log("univ.nuniversity_id:",univ.ncourse_id);
                		...
                		if(pdfv != undefined){
                    		for (const optid in pdfv) {
								...
								// 	options_section += '<input type="radio" for="regForm" value="'+optv.noption_id+'" name="seloptions" seluniversity="'+course_id+'">';
								    if('<?php echo $optid_param; ?>' == optv.noption_id){ 
                                        options_section += '<input type="radio" for="regForm" value="'+optv.noption_id+'" name="seloptions" seluniversity="'+course_id+'" checked="true">';
                        		    }
                        		    else{
                        		        options_section += '<input type="radio" for="regForm" value="'+optv.noption_id+'" name="seloptions" seluniversity="'+course_id+'">';
                        		    }
...
function sendPDF(vname,vphone,emailid,pdfurl){
    ...
    data.append('pdfFiletext', search_params.get('pg'));
