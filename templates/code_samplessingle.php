...
// echo "RNB{";
// echo $subjectname_param;
// echo "<br>";
// echo $optionname_param;
// echo "}Infotech";
?>
<style>
/* Style the form */
...
$opt_row = $wpdb->get_results("SELECT noption_id, voption_title, noption_order, voption_type from $table_name2 WHERE voption_type='course' AND vslug='".$optionname_param."' order by noption_order", ARRAY_A);

// echo "SELECT noption_id, voption_title, noption_order, voption_type from $table_name2 WHERE voption_type='course' AND vslug='".$optionname_param."' order by noption_order";
// echo "<pre>"; print_r($opt_row); echo "</pre>";
...
$optionrows = $wpdb->get_results("SELECT noption_id, voption_title, noption_order, voption_type, vslug from $table_name2 WHERE voption_type='course' order by noption_order");
...
$heading = '<a href="'.$dbsiteurl.'/research-writing-samples">Research Writing Sample</a> - '.$subject_slug_rows[0]['vcourse_title'];
...
//      tab_type = e.path[0].attributes[2].name;
    ...
    if (vemailSet)  {
		...
		var notihtml = '<div class="col-12 text-start"><div class="alert alert-warning text-center" role="alert"><strong><a href="http://phdguides.org/university-formats-guidelines/" target="_blank" class="btn btn-primary" style="font-weight:bold;">Also see our University Formats & Guidelines</a></strong></div></div>';
		...
		options_data_array.map((optv,opti) => {
			if(optv.voption_type == 'course'){
                	...
                	pdfdataa.forEach((pdfv,pdfi) => {
                		if(pdfv != undefined){
                        	// console.log("optv.noption_id",pdfv);
...
window.addEventListener("click", (e) => {
    ...
    if(ele_type == 'radio'){
    	...
    	new_url = current_url.origin+"/research-writing-samples/<?php echo $subjectname_param; ?>/"+e.originalTarget.attributes[2].value;
...
function sendPDF(vname,vphone,emailid,pdfurl){
    ...
    data.append('pdfFiletext', '<?php echo $subjectname_param." (".$optionname_param.")"; ?>');
