...
function rnb_course_upload() {
    ...
    if (isset($_POST['insert'])) {
        ...
        $wpdb->insert(
                $table_name2,
                array('nuniversity_id' => $id, 'vpdf_data' => json_encode($ar), 'vimage_data' => json_encode($ar2), 'vpdf_type' => 'samples'),
                array('%s', '%s', '%s', '%s')
        );
    }
        ...
        $university = $wpdb->get_results($wpdb->prepare("SELECT t1.ncourse_id, t1.vcourse_title, t2.npdf_id, t2.vpdf_data, t2.vimage_data FROM $table_name1 t1 LEFT JOIN $table_name2 t2 ON t2.nuniversity_id = t1.ncourse_id WHERE t1.ncourse_id=%s AND t2.vpdf_type='samples'", $id));
        ...
        <?php } else { ?>
            ...
            <form method="post" action="<?php echo $_SERVER['REQUEST_URI']; ?>" enctype="multipart/form-data">
                ...
                <tr>
                    ...
                    <th class="manage-column ss-list-width" width="30%" colspan="2">Option Title</th>
                    <th class="manage-column ss-list-width" width="30%" colspan="2">Upload PDF File</th>
                    <th width="25%">&nbsp;</th>
                </tr>
